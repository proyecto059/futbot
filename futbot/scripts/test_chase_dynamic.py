...
BALL_VISIBLE_MIN_RADIUS = float(os.environ.get("BALL_VISIBLE_MIN_RADIUS", "5.0"))
...
LINE_ATTACK_COMMIT_MS = int(os.environ.get("LINE_ATTACK_COMMIT_MS", "550"))
...
try:
    while True:
        ...
        if line_cooldown <= 0 and line_phase == "":
            ...
            if line_confirmed:
                line_attack_allowed = should_controlled_attack_through_line(
                    ball,
                    goal_cx=target_goal_cx_now,
                    line_detected=True,
                    now=now,
                    last_ball_cx=last_valid_cx,
                    last_ball_r=last_valid_r,
                    last_seen_t=last_seen_t,
                    min_radius=DRIBBLE_MIN_RADIUS,
                    max_ball_goal_dx=120.0,
                    max_age_s=GOAL_COAST_MAX_BALL_AGE,
                ) or should_attack_through_line(
                    ball,
                    goal_cx=target_goal_cx_now,
                    line_detected=True,
                    min_radius=BALL_VISIBLE_MIN_RADIUS,
                ...
                if line_attack_allowed:
                    dribble_until = max(
                        dribble_until,
                        now + LINE_ATTACK_COMMIT_MS / 1000.0,
                    )
                    if tick % 5 == 0:
                        print(
                            "--- LINE suppressed: controlled attack cx={} last={:.0f} goal={} ---".format(
