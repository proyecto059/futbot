...
Estado estadoActual = BUSCANDO;

unsigned long tiempoAnterior = 0;
const unsigned long intervaloGiro = 80;  // velocidad del giro
...
void loop() {
  ...
  if (huskylens.available()) {
      ...
      if (result.ID == 1 && result.width > 40) {
          pelotaDetectada = true;
      }
  }

  // 🎯 PRIORIDAD ABSOLUTA A LA PELOTA
  if (pelotaDetectada) {
...
void girarNoBloqueante() {

  unsigned long tiempoActual = millis();

  if (tiempoActual - tiempoAnterior >= intervaloGiro) {
