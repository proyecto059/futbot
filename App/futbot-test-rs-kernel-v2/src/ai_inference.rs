...
pub(crate) fn should_log_ai_error(count: u64) -> bool {
    count <= 5 || count % 50 == 0
}
...
#[cfg(any(test, feature = "ai"))]
pub(crate) fn parse_output_layout(shape: &[i64]) -> anyhow::Result<(usize, usize, AIOutputFormat)> {
    let ndims = shape.len();
...
#[cfg(any(test, feature = "ai"))]
pub(crate) fn parse_row_conf_class(flat: &[f32], base: usize, format: AIOutputFormat) -> (f32, f32) {
    match format {
...
mod inner {
    ...
    pub(crate) fn fill_tensor_from_rgb(rgb: &Mat, tensor: &mut Array4<f32>) -> Result<()> {
...
#[cfg(feature = "ai")]
pub(crate) use inner::fill_tensor_from_rgb;
