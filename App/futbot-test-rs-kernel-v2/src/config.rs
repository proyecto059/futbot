...
// ── Servo tilt tracking (camera up/down) ─────────────────────────────────────
pub fn servo_tilt_enabled() -> bool {
    env_bool("SERVO_TILT_ENABLED", true)
}

pub fn servo_pwm_bcm() -> u8 {
    env_u8("SERVO_PWM_BCM", 18)
}

pub fn servo_min_angle() -> f64 {
    env_f64("SERVO_MIN_ANGLE", 20.0)
}

pub fn servo_max_angle() -> f64 {
    env_f64("SERVO_MAX_ANGLE", 160.0)
}

pub fn servo_home_angle() -> f64 {
    env_f64("SERVO_HOME_ANGLE", 90.0)
}

pub fn servo_step_deg() -> f64 {
    env_f64("SERVO_STEP_DEG", 2.0)
}

pub fn servo_dead_zone_y() -> i32 {
    env::var("SERVO_DEAD_ZONE_Y")
        .ok()
        .and_then(|v| v.parse::<i32>().ok())
        .unwrap_or(15)
}

pub fn servo_invert() -> bool {
    env_bool("SERVO_INVERT", false)
}

pub fn servo_hold_no_ball_frames() -> u32 {
    env_u32("SERVO_HOLD_NO_BALL_FRAMES", 10)
}

pub fn servo_return_home() -> bool {
    env_bool("SERVO_RETURN_HOME", true)
}
