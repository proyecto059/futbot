...
def test_yolo_image_size_matches_current_ncnn_export():
    ...
    assert resolve_yolo_model_path().as_posix().endswith(
...
def test_as_detection_rows_decodes_yolov8_channels_first_output():
    from vision.utils.yolo_backend_factory import _as_detection_rows

    output = np.array(
        [
            [100.0, 102.0, 200.0],  # cx
            [80.0, 82.0, 150.0],  # cy
            [40.0, 40.0, 20.0],  # w
            [20.0, 20.0, 20.0],  # h
            [0.10, 0.20, 0.80],  # ball score
            [0.90, 0.70, 0.20],  # robot score
        ],
        dtype=np.float32,
    )
    ...
    assert rows[0].tolist() == [80.0, 70.0, 120.0, 90.0, pytest.approx(0.90), 1.0]
    assert rows[1].tolist() == [190.0, 140.0, 210.0, 160.0, pytest.approx(0.80), 0.0]


def test_as_detection_rows_decodes_batched_yolov8_output():
    from vision.utils.yolo_backend_factory import _as_detection_rows
...
def test_as_detection_rows_suppresses_overlapping_yoloe_robot_subclasses():
    from vision.utils.yolo_backend_factory import _as_detection_rows
