...
def should_accept_tracked_ball_measurement(
    ...
    if age > float(max_recent_age_s):
        return True
    jump = abs(float(ball.get("cx", 0.0)) - float(last_cx))
    ...
    if (
        ...
        weak_plausible_jump = float(frame_width) * 0.14 + float(max_jump_per_s) * age * 0.25
        if jump <= weak_plausible_jump:
            return True
...
def should_controlled_attack_through_line(
    ball: dict | None,
    goal_cx: float | None,
    line_detected: bool,
    now: float,
    ...
    if should_attack_through_line(
        ...
        max_ball_goal_dx=max_ball_goal_dx,
    ):
        return True
    ...
    if float(now) - float(last_seen_t) > float(max_age_s):
        return False
    if float(last_ball_r) < float(min_radius):
        return False
    return abs(float(last_ball_cx) - float(goal_cx)) <= float(max_ball_goal_dx)
