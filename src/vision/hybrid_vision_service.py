...
JSON-serializable con ball / hsv_ball / yolo_ball / robots / goals / line / ts / debug.
...
Ejemplo:
    ...
    try:
        while running:
            snap = vision.tick()
            # snap['ball'], snap['hsv_ball'], snap['yolo_ball'], snap['goals']['blue_cx'], ...
    finally:
        vision.close()
"""
...
from vision.dto.ball_dto import ball_to_dict
...
class HybridVisionService:
    ...
    def tick(
        ...
        return {
            "ts": now_ts,
            "ball": ball_to_dict(ball) if ball else None,
            "hsv_ball": ball_to_dict(hsv_ball) if hsv_ball else None,
            "yolo_ball": ball_to_dict(yolo_ball) if yolo_ball else None,
