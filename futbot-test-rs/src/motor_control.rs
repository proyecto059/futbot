...
impl PIDController {
    pub fn new(kp: f64, ki: f64, kd: f64, max_output: f64) -> Self {
        PIDController { kp, ki, kd, max_output, integral: 0.0, prev_error: 0.0 }
    }
...
impl MotorController {
    ...
    pub fn setup(&mut self) {
        ...
        match Self::try_setup() {
            Ok(motors) => {
                ...
                log::info!("[motors] GPIO initialized (BCM: A_DIR={} A_PWM={} B_DIR={} B_PWM={})",
                    MOTOR_A_DIR, MOTOR_A_PWM, MOTOR_B_DIR, MOTOR_B_PWM);
            }
            Err(e) => {
                log::warn!("[motors] GPIO not available ({}) — running without motor control", e);
            }
        }
        #[cfg(not(target_os = "linux"))]
    ...
    fn try_setup() -> anyhow::Result<GpioMotors> {
        ...
        Ok(GpioMotors { gpio, dir_a, dir_b, pwm_a, pwm_b })
    }
    ...
    #[cfg(target_os = "linux")]
    fn set_motor_direction(dir: &mut rppal::gpio::OutputPin, pwm: &mut rppal::gpio::OutputPin, speed: f64) {
        // Direction
        if speed >= 0.0 {
            dir.set_high();
        } else {
