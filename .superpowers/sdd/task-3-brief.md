...
- Consume: `Config` de `config.py` (ya existe del Task 1)
...
```bash
cd futbot; python -m pytest tests/test_vision.py -v
```
Esperado: FAIL — `ModuleNotFoundError: No module named 'vision'`
...
cd futbot; python -m pytest tests/test_vision.py -v
```
Esperado: 4 passed (dataclass + empty + HSV ball + no ball)
