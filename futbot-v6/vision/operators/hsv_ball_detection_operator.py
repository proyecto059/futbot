...
class HsvBallDetectionOperator:
    ...
    def detect(self, frame: np.ndarray, now_ts: float) -> Optional[BallDto]:
        ...
        hsv = cv2.cvtColor(cv2.GaussianBlur(corrected, (5, 5), 0), cv2.COLOR_BGR2HSV)
        ...
        if tan_best is None:
            best = strict_best
        elif strict_best is not None and strict_best[2] >= tan_best[2] * 2.0:
            best = strict_best
            mode = "strict"
    ...
    def _update_adaptive_thresholds(self, hsv: np.ndarray, relaxed: bool) -> None:
        ...
        for tier_name, (v_threshold, tier_sat) in ADAPTIVE_SAT_TIERS.items():
            if v_median < v_threshold:
                strict_sat = tier_sat
                relaxed_sat = max(50, tier_sat - 20)
                break
    ...
    def _tan_candidate_mask(hsv: np.ndarray) -> np.ndarray:
        mask = cv2.inRange(
            ...
            np.array((76, 190, 220), dtype=np.uint8),
        )
    ...
    def _scan_contours(
        ...
        candidates = []
        base_min_area = max(20.0, float(BALL_MIN_AREA) * 0.4)
        ...
        min_radius = max(2.0, float(BALL_MIN_RADIUS) * 0.6)
        effective_min_circ = (
        ...
        for _score, x, y, radius in candidates[:3]:
            ...
            if not self._is_valid_ball_hue(patch_median_h):
                continue
    ...
    @staticmethod
    def _is_valid_ball_hue(hue: int) -> bool:
        return ADAPTIVE_ORANGE_HUE_MIN <= int(hue) <= 76
